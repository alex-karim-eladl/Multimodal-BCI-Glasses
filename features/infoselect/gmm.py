...
def gmm_scores(X_train, X_val, k=3, covariance_type='full', reg_covar=1e-06, random_state=42):
    ...
    clf = mixture.GaussianMixture(
        n_components=k, covariance_type=covariance_type, reg_covar=reg_covar, random_state=random_state, max_iter=1000
    )
    clf.fit(X_train)
...
def get_gmm(
    ...
    y,
    y_cat=False,
    num_comps=[2, 5, 10, 15, 20, 30, 40, 50],
    val_size=0.33,
    reg_covar=1e-06,
    ...
    if y_cat:
        ...
        for c in classes:
            ...
            X_gmm_train, X_gmm_val, _, _ = train_test_split(
                X[y == c], X[y == c], test_size=val_size, random_state=random_state
            )
            scores = np.array(
                [
                    gmm_scores(
            ...
            # Training GMMs
            gmm[c] = mixture.GaussianMixture(
                n_components=k_star, covariance_type=covariance_type, reg_covar=reg_covar, random_state=random_state
            )
            gmm[c].fit(X[y == c])
