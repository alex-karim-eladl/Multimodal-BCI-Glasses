"""
from https://github.com/felipemaiapolo/infoselect
"""
...
def MI_gmm_reg(X, y, gmm, feat, eps=10**-50):
    ...
    feat2 = (
        [0] + [f + 1 for f in feat]
    )  # feat2 includes y in addition de X[:,feat]. PS: we sum 1 because when we train GMM, the first variable is always y

    ### Calculating log-likelihood with samples (x_i,y_i)
    like = np.zeros(n)
    for c in range(components):
        like += gmm.weights_[c] * multivariate_normal.pdf(
            Z[:, feat2], gmm.means_[c][feat2], gmm.covariances_[c][feat2][:, feat2]
        )
    ...
    for c in range(components):
        like += gmm.weights_[c] * multivariate_normal.pdf(
            Z[:, feat2[1:]], gmm.means_[c][feat2[1:]], gmm.covariances_[c][feat2[1:]][:, feat2[1:]]
        )
...
def MI_gmm_class(X, y, gmm, feat, eps=10**-50):
    ...
    for c in classes:
        ...
        for comp in range(gmm[c].n_components):
            like_aux[y == c] += gmm[c].weights_[comp] * multivariate_normal.pdf(
                X[y == c][:, feat], gmm[c].means_[comp][feat], gmm[c].covariances_[comp][feat][:, feat]
            )
    ...
    for c in classes:
        ...
        like_aux = np.zeros(n)
        for comp in range(gmm[c].n_components):
            like_aux += gmm[c].weights_[comp] * multivariate_normal.pdf(
                X[:, feat], gmm[c].means_[comp][feat], gmm[c].covariances_[comp][feat][:, feat]
            )
