...
from config import *
from preprocessing.filters import fir_filter


def cnr(df):

    idx = pd.MultiIndex.from_arrays([3*TIMINGS, 2*STIMULI], names=['timing', 'stimulus'])
    cnr_df = pd.DataFrame(index=idx, columns=df.columns).sort_index()
    for stimulus in STIMULI:
        for timing in TIMINGS:
            groups = df.loc[(timing, stimulus)].groupby(['round', 'task'])
            cnrs = []
            for rnd in range(10):
                math_df = groups.get_group((str(rnd), 'math'))
                rest_df = groups.get_group((str(rnd), 'rest'))
                
                num = math_df.mean() - rest_df.mean()
                den = np.sqrt(math_df.var() + rest_df.var())
                # print(num.to_numpy(), den.to_numpy())
                cnr = num.to_numpy() / den.to_numpy()
                cnrs.append(cnr)
                
            mean = np.array(cnrs).mean(axis=0)
            cnr_df.loc[(timing, stimulus)] = mean
    return cnr_df
...
def SQI(OD1, OD2, oxy, dxy, Fs):
    ...
    if (
        np.any(OD1 < thrLow_intensity)
        or np.any(OD1 > thrUp_intensity)
        or np.any(OD2 < thrLow_intensity)
        or np.any(OD2 > thrUp_intensity)
    ):
        SQIscore = 1
        return SQIscore
